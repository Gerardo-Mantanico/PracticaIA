...
const DASH_FLEXIBLE_REGEX = /^(\d{4})-(\d{1,2})-(\d{1,2})$/;
...
const normalizeEntryDate = (value) => {
  ...
  const slashYmd = SLASH_YMD_REGEX.exec(raw);
  if (slashYmd) {
    return buildDateFromParts(slashYmd[1], slashYmd[2], slashYmd[3]);
  }

  const slashDmy = SLASH_DMY_REGEX.exec(raw);
  if (slashDmy) {
    return buildDateFromParts(slashDmy[3], slashDmy[2], slashDmy[1]);
  }

  const dashFlexible = DASH_FLEXIBLE_REGEX.exec(raw);
  if (dashFlexible) {
