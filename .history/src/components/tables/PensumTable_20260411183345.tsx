...
export default function PensumTable() {
  ...
  const filteredPensums = useMemo(() => {
    ...
    return pensums.filter((pensum) => {
      const haystack = [
        pensum.name,
        pensum.careerName,
        careerMap.get(String(resolveCareerId(pensum))) ?? "",
      ]
        .join(" ")
        .toLowerCase();
  ...
  const handleOpenDetail = (pensum: Pensum) => {
    ...
    setCourseFormData({
      ...initialCourseForm,
      pensumId: undefined as never,
    } as PensumCourseFormData);
    setShowAddCourseForm(false);
    openDetailModal();
  };

  const handleSavePensum = async (event: React.FormEvent) => {
  ...
  const handleSaveCourseToPensum = async (event: React.FormEvent) => {
    event.preventDefault();
  ...
  const handleDeletePensum = async (pensumId: number) => {
    const confirmed = window.confirm("¿Eliminar este pensum?");
    if (!confirmed) return;
        ...
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {filteredPensums.map((pensum) => {
            ...
            return (
              ...
              >
                <div className="border-b border-gray-100 bg-gradient-to-br from-brand-50 to-white p-5 dark:border-gray-800 dark:from-brand-500/15 dark:to-gray-900">
