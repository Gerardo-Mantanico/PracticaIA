...
const renderPensumCourseState = (
  ...
  return pensumCourses.map((course) => {
    ...
    return (
      <div key={course.id || `${courseCode}-${course.semester}`} className="rounded-sm border border-orange-300 bg-orange-200/70">
        <div className="grid grid-cols-[58px_1fr_58px] overflow-hidden">
          <div className="grid grid-rows-2 bg-orange-500 text-white">
            <div className="flex items-center justify-center text-sm font-semibold">{String(courseCode).padStart(4, "0")}</div>
            <div className="flex items-center justify-center border-t border-orange-300 text-sm font-semibold">{credits}</div>
          </div>
          ...
          <div className="flex items-center justify-center bg-orange-500 text-white">
            <span className="text-xl leading-none">•</span>
          </div>
        </div>

        <div className="grid gap-1 border-t border-orange-300 bg-white/70 px-3 py-2 text-[11px] text-gray-700">
          <p>
            <span className="font-semibold">Prerequisitos:</span>{" "}
            {prereqs.length > 0 ? prereqs.map(formatRelatedCourse).join(", ") : "Ninguno"}
          </p>
          <p>
            <span className="font-semibold">Postrequisitos:</span>{" "}
            {postreqs.length > 0 ? postreqs.map(formatRelatedCourse).join(", ") : "Ninguno"}
          </p>
        </div>
      </div>
    );
