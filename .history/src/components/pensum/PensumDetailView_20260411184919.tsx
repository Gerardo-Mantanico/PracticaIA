...
import { ArrowRightIcon, PlusIcon, EyeIcon } from "@/icons";
...
export default function PensumDetailView({ pensumId }: { pensumId: number }) {
  ...
  const handleSaveCourseToPensum = async (event: React.SyntheticEvent<HTMLFormElement>) => {
    ...
    if (!selectedPensum) {
      toast.error("No se encontró el pensum seleccionado");
      return;
    }
  ...
  if (!selectedPensum) {
    return (
      <div className="flex flex-col items-center justify-center gap-4 rounded-2xl border border-dashed border-gray-300 bg-white p-12 dark:border-gray-700 dark:bg-white/3">
        <p className="text-center text-gray-500 dark:text-gray-400">No se encontró el pensum seleccionado.</p>
              ...
              <div className="mt-5 grid gap-4 md:grid-cols-[1.1fr_0.9fr]">
                ...
                <div className="rounded-2xl bg-white p-4 shadow-theme-xs dark:bg-gray-900/70">
                  ...
                  <div className="mt-3 space-y-2 text-sm text-gray-700 dark:text-gray-300">
                    <p>
                      <span className="font-medium text-gray-900 dark:text-white">Carrera:</span> {selectedCareerName}
                    </p>
                    <p>
                      <span className="font-medium text-gray-900 dark:text-white">Créditos:</span> {selectedPensum?.creditsNeeded ?? 0}
                    </p>
                    <p>
                      <span className="font-medium text-gray-900 dark:text-white">Estado:</span> {selectedPensum?.active ? "Activo" : "Inactivo"}
                    </p>
                  </div>
                </div>
              </div>
