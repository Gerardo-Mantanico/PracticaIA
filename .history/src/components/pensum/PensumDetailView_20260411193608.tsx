...
export default function PensumDetailView({ pensumId }: Readonly<{ pensumId: number }>) {
  ...
  const resolvedPensumId = useMemo(() => {
    ...
    const fromList = resolvePensumId((pensums[0] ?? {}) as { id?: number; pensumId?: number; pensumCode?: number });
    return Number.isFinite(fromList) && fromList > 0 ? fromList : 0;
  }, [pensumId, params, pensums]);
